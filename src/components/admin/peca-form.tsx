...
export default function PecaForm({ printers, filaments, initialData }: PecaFormProps) {
  ...
  // Live calculation Memo
  const custosCalculados = useMemo(() => {
    return calcularCustoPeca({
      material: selectedFilament
        ? { precoPorKg: selectedFilament.precoPorKg, pesoGramas }
        : undefined,
      energia: selectedPrinter
        ? {
            consumoWatts: selectedPrinter.consumoWatts,
            tempoHoras: tempoHorasImpressao,
            tarifaEnergiaKwh,
          }
        : undefined,
      depreciacao: selectedPrinter
        ? {
            precoImpressora: selectedPrinter.preco,
            vidaUtilHoras: selectedPrinter.vidaUtilHoras,
            tempoHoras: tempoHorasImpressao,
          }
        : undefined,
      pintura: {
        tempoHoras: tempoHorasPintura,
        valorHoraMaoDeObra,
        custoTintas,
      },
      embalagem: {
        custoUnitario: custoUnitarioEmbalagem,
      },
    ...
    <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          <Link
            href="/admin/pecas"
            className="inline-flex items-center gap-2 text-xs font-semibold text-teal-600 dark:text-teal-400 hover:underline mb-2"
          >
            <ArrowLeft className="w-4 h-4" /> Voltar para Lista de Peças
          </Link>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">
            {initialData ? `Editar Peça: ${initialData.nome}` : "Cadastrar Nova Peça 3D & Calculadora"}
          </h1>
          <p className="text-slate-600 dark:text-slate-400 text-xs sm:text-sm mt-1">
            Preencha os parâmetros de impressão, insumos e pintura para cálculo preciso de custos.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <Button variant="secondary" type="button" onClick={() => router.push("/admin/pecas")}>
            Cancelar
          </Button>
          <Button variant="primary" type="submit" disabled={loading} className="px-6">
            <Save className="w-4 h-4" />
            {loading ? "Salvando..." : initialData ? "Salvar Alterações" : "Cadastrar Peça"}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-4 shadow-md transition-colors">
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100 pb-2 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              ...
              <label className="flex items-center gap-2 text-xs font-medium cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-slate-300 dark:border-slate-700 text-teal-500 focus:ring-teal-500"
                />
                <span className={publicada ? "text-teal-600 dark:text-teal-400 font-bold" : "text-slate-500 dark:text-slate-400"}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-teal-500"
                />
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs text-slate-500 dark:text-slate-400 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-100 dark:file:bg-slate-800 file:text-teal-600 dark:file:text-teal-400 hover:file:bg-slate-200 dark:hover:file:bg-slate-700 cursor-pointer"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-teal-500"
                >
            ...
            {imagePreview && (
              <div className="mt-2">
                <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-1">Pré-visualização da Imagem:</p>
                <div className="w-24 h-24 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-4 shadow-md transition-colors">
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 rounded-2xl space-y-2">
              ...
              <input
                ...
                className="w-full text-xs text-slate-500 dark:text-slate-400 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-teal-500/10 file:text-teal-700 dark:file:text-teal-300 hover:file:bg-teal-500/20 cursor-pointer"
              />
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-4 shadow-md transition-colors">
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100 pb-2 border-b border-slate-200 dark:border-slate-800">
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sticky top-8 shadow-xl space-y-6 transition-colors">
            <div className="flex items-center gap-2 text-teal-600 dark:text-teal-400 font-bold border-b border-slate-200 dark:border-slate-800 pb-3">
            ...
            <div className="space-y-3 text-xs">
              <div className="flex justify-between items-center text-slate-700 dark:text-slate-300">
                <span className="text-slate-500 dark:text-slate-400">Filamento ({pesoGramas}g):</span>
                <span className="font-semibold">{formatarMoeda(custosCalculados.custoMaterial)}</span>
              </div>

              <div className="flex justify-between items-center text-slate-700 dark:text-slate-300">
                <span className="text-slate-500 dark:text-slate-400">Energia Elétrica ({tempoHorasImpressao}h):</span>
                <span className="font-semibold">{formatarMoeda(custosCalculados.custoEnergia)}</span>
              </div>

              <div className="flex justify-between items-center text-slate-700 dark:text-slate-300">
                <span className="text-slate-500 dark:text-slate-400">Depreciação Máquina:</span>
                <span className="font-semibold">{formatarMoeda(custosCalculados.custoDepreciacao)}</span>
              </div>

              <div className="flex justify-between items-center text-slate-700 dark:text-slate-300 pt-2 border-t border-slate-200 dark:border-slate-800/80 font-medium">
                <span className="text-slate-800 dark:text-slate-300">Subtotal Impressão:</span>
                <span className="text-cyan-600 dark:text-cyan-400">{formatarMoeda(custosCalculados.custoImpressaoTotal)}</span>
              </div>

              {custosCalculados.custoPintura > 0 && (
                <div className="flex justify-between items-center text-slate-700 dark:text-slate-300">
                  <span className="text-slate-500 dark:text-slate-400">Pintura & Mão de Obra:</span>
              ...
              {custosCalculados.custoEmbalagem > 0 && (
                <div className="flex justify-between items-center text-slate-700 dark:text-slate-300">
                  <span className="text-slate-500 dark:text-slate-400">Embalagem:</span>
              ...
              <div className="pt-3 border-t border-slate-200 dark:border-slate-800/80">
                <div className="flex justify-between items-center">
                  <span className="text-slate-800 dark:text-slate-200 font-bold text-sm">CUSTO TOTAL DE PRODUÇÃO:</span>
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      className={`w-24 px-3 py-1.5 border rounded-lg text-sm font-bold text-center focus:outline-none ${
                        isLowMargin
                          ? "bg-white dark:bg-slate-900 border-amber-500/50 text-amber-700 dark:text-amber-300 focus:border-amber-400"
                          : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 focus:border-teal-500"
                      }`}
                    />
                    <span className="text-xs text-slate-500 dark:text-slate-400">% de lucro sobre custo</span>
                  </div>
                  ...
                  <div className="pt-2 border-t border-slate-200 dark:border-slate-800/80">
                    ...
                    <div
                      className={`text-2xl font-black mt-0.5 ${
                        isLowMargin ? "text-amber-600 dark:text-amber-400" : "text-emerald-600 dark:text-emerald-400"
                      }`}
                    >
