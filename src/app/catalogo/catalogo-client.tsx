...
export default function CatalogoClientPage({
  ...
  return (
    <div className="space-y-8 sm:space-y-10 animate-in fade-in duration-300">
      {/* Hero Banner */}
      <div className="relative rounded-3xl bg-gradient-to-r from-slate-100 via-white to-slate-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950 border border-slate-200 dark:border-slate-800 p-6 sm:p-12 overflow-hidden shadow-xl dark:shadow-2xl transition-colors">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <h1 className="text-3xl sm:text-5xl font-black text-slate-900 dark:text-slate-100 tracking-tight leading-tight">
            Galeria de Peças <span className="text-teal-500 dark:text-teal-400">&</span> Colecionáveis
          </h1>
      ...
      {/* Search & Filter Bar */}
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl backdrop-blur-md shadow-sm transition-colors">
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-teal-500"
            />
          </div>
          ...
          <select
            ...
            className="w-full sm:w-auto px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-700 dark:text-slate-300 font-semibold focus:outline-none focus:border-teal-500 cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredPecas.map((peca) => (
            ...
            >
              <div>
                {/* Photo container */}
                <div className="relative h-56 bg-slate-100 dark:bg-slate-950 overflow-hidden flex items-center justify-center">
                  {peca.fotoUrl ? (
                    <img
                      src={peca.fotoUrl}
                      alt={peca.nome}
                    ...
                    <div className="text-center p-4">
                      <Box className="w-12 h-12 text-slate-400 dark:text-slate-700 mx-auto mb-2" />
                      <span className="text-xs text-slate-500">Impressão 3D</span>
                    </div>
                  )}

                  {peca.categoria && (
                    <span className="absolute top-3 left-3 bg-white/90 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 text-[10px] font-semibold px-2.5 py-1 rounded-md backdrop-blur-md">
                      {peca.categoria}
                    </span>
                  )}
                </div>
                ...
                {/* Info Content */}
                <div className="p-5">
                  <h3 className="font-bold text-lg text-slate-900 dark:text-slate-100 group-hover:text-teal-500 dark:group-hover:text-teal-400 transition-colors line-clamp-1">
                    {peca.nome}
                  </h3>
                  {peca.descricao ? (
              ...
              {/* Action link footer */}
              <div className="p-4 border-t border-slate-200 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-950/40 flex items-center justify-between text-xs font-semibold text-teal-600 dark:text-teal-400 group-hover:text-teal-500 dark:group-hover:text-teal-300">
