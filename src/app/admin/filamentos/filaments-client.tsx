      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-3">
            <Boxes className="w-7 h-7 sm:w-8 sm:h-8 text-cyan-500 dark:text-cyan-400" /> Filamentos & Resinas
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {initialFilaments.map((filament) => {
            ...
            return (
              <div
                key={filament.id}
                className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 relative group hover:border-cyan-500/40 transition-all shadow-md dark:shadow-lg flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center font-bold text-slate-800 dark:text-slate-200 text-xs">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {filament.marca && (
                            <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">{filament.marca}</span>
                          )}
                        </div>
                        <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 mt-1 flex items-center gap-1.5">
                          <Palette className="w-3.5 h-3.5 text-cyan-500 dark:text-cyan-400" /> {filament.cor}
                        </h3>
                      </div>
                    </div>
                    <div className="flex items-center gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-cyan-500 dark:hover:text-cyan-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors"
                        title="Editar"
                      >
                  ...
                  <div className="space-y-3 text-xs text-slate-700 dark:text-slate-300 border-t border-slate-200 dark:border-slate-800/80 pt-4">
                    {/* Stock Progress Bar */}
                    {(() => {
                      ...
                      return (
                        <div className="space-y-1.5 bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-200 dark:border-slate-800/80">
                          <div className="flex justify-between items-center text-[11px]">
                            <span className="text-slate-500 dark:text-slate-400 font-semibold flex items-center gap-1">
                              Estoque Restante:
                            </span>
                            <span className={`font-bold font-mono ${isLowStock ? "text-rose-600 dark:text-rose-400" : "text-emerald-600 dark:text-emerald-400"}`}>
                          ...
                          {isLowStock && (
                            <div className="text-[10px] font-bold text-rose-600 dark:text-rose-400 pt-0.5 flex items-center justify-between">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 dark:text-slate-400">Preço do Carretel / Kg</span>
                      <span className="font-bold text-slate-900 dark:text-slate-100">{formatarMoeda(filament.precoPorKg)} / kg</span>
                    </div>

                    {/* Price History Timeline */}
                    {filament.priceHistory && filament.priceHistory.length > 0 && (
                      <div className="pt-2 border-t border-slate-200 dark:border-slate-800/50 space-y-1">
                        <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block">
                          Histórico de Preços Anteriores:
                        </span>
                        <div className="space-y-1 max-h-24 overflow-y-auto pr-1">
                          {filament.priceHistory.map((h: any) => (
                            <div key={h.id} className="flex justify-between text-[11px] text-slate-500 dark:text-slate-400">
                              <span>{new Date(h.data).toLocaleDateString("pt-BR")}</span>
                              <span className="font-mono line-through text-rose-500/80 dark:text-rose-400/80">{formatarMoeda(h.precoPorKg)}/kg</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-800/50">
                      <span className="text-slate-500 dark:text-slate-400 font-medium">Custo por Grama</span>
                      <span className="font-bold text-cyan-600 dark:text-cyan-400">{formatarMoeda(precoPorGrama)} / g</span>
                    </div>
                  </div>
                </div>
              </div>
