...
export default function PecasClientPage({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-100 tracking-tight flex items-center gap-3">
            <Box className="w-8 h-8 text-teal-400" /> Peças & Produção
          </h1>
      ...
      {/* Filters Bar */}
      <div className="flex flex-wrap items-center gap-4 bg-slate-900 border border-slate-800 p-4 rounded-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPecas.map((peca) => {
            ...
            return (
              <div
                key={peca.id}
                className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden group hover:border-teal-500/40 transition-all flex flex-col justify-between shadow-lg"
              >
                {/* Photo & Badge header */}
                <div className="relative h-48 bg-slate-950 border-b border-slate-800/80 flex items-center justify-center overflow-hidden">
                  {peca.fotoUrl ? (
                    <img
                      src={peca.fotoUrl}
                      alt={peca.nome}
                  ...
                  {peca.categoria && (
                    <span className="absolute top-3 left-3 bg-slate-900/90 border border-slate-700/80 text-slate-300 text-[10px] font-semibold px-2.5 py-1 rounded-md backdrop-blur-md">
                      {peca.categoria}
                    </span>
                  )}
                </div>
                ...
                <div className="p-5 flex-1 space-y-4">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-500 font-medium">Status de Produção:</span>
                    <select
                      value={peca.status}
                      onChange={(e) => handleStatusChange(peca.id, e.target.value)}
                      className={`px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-950 border border-slate-800 focus:outline-none ${
                        peca.status === "pronta"
                  ...
                  <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-3.5 space-y-1.5 text-xs">
                    <div className="flex justify-between text-slate-400">
                      ...
                      <span className="font-medium text-slate-300">{formatarMoeda(custos.custoImpressaoTotal)}</span>
                    </div>
                    {custos.custoPintura > 0 && (
                      <div className="flex justify-between text-slate-400">
                        <span>Custo de Pintura:</span>
                        <span className="font-medium text-slate-300">{formatarMoeda(custos.custoPintura)}</span>
                      </div>
                    )}
                    {custos.custoEmbalagem > 0 && (
                      <div className="flex justify-between text-slate-400">
                        <span>Embalagem:</span>
                        <span className="font-medium text-slate-300">{formatarMoeda(custos.custoEmbalagem)}</span>
                      </div>
                    )}
                    <div className="flex justify-between pt-2 border-t border-slate-800 font-bold">
                      <span className="text-slate-300">Preço Sugerido:</span>
                      <span className="text-teal-400">{formatarMoeda(custos.precoSugerido)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 border-t border-slate-800/80 bg-slate-900/40 flex items-center justify-between gap-2">
                  <button
                    ...
                    className="flex-1 px-3 py-2 bg-teal-500/10 border border-teal-500/20 hover:bg-teal-500/20 text-teal-300 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-all"
                  >
